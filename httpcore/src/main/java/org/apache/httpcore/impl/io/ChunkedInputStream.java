...
/**
 * Implements chunked transfer coding. The content is received in small chunks. Entities transferred using
 * this input stream can be of unlimited length. After the stream is read to the end, it provides access to
 * the trailers, if any. <p> Note that this class NEVER closes the underlying stream, even when close gets
 * called.  Instead, it will read until the "end" of its chunking on close, which allows for the seamless
 * execution of subsequent HTTP 1.1 requests, while not requiring the client to remember to read the entire
...
public class ChunkedInputStream
  extends InputStream {
    ...
    /**
     * <p> Returns all the data in a chunked stream in coalesced form. A chunk is followed by a CRLF. The
     * method returns -1 as soon as a chunksize of 0 is detected.</p>
     *
     * <p> Trailer headers are read automatically at the end of the stream and can be obtained with the
     * getResponseFooters() method.</p>
     *
    ...
    /**
     * Expects the stream to start with a chunksize in hex with optional comments after a semicolon. The line
     * must end with a CRLF: "a3; some comment\r\n" Positions the stream at the start of the next line.
     */
    ...
    private void parseTrailerHeaders() throws IOException {
        try {
            this.footers = AbstractMessageParser.parseHeaders(in, constraints.getMaxHeaderCount(),
              constraints.getMaxLineLength(), null);
