...
/**
 * Basic implementation for formatting header value elements. Instances of this class are stateless and
...
public class BasicHeaderValueFormatter
  implements HeaderValueFormatter {
     ...
     * BasicHeaderValueFormatter} is not a singleton, there can be many instances of the class itself and of
    ...
    /**
     * Special characters that can be used as separators in HTTP parameters. These special characters MUST be
     * in a quoted string to be used within a parameter value .
     */
     ...
     * @param quote {@code true} to always format with quoted values, {@code false} to use quotes only when
     *   necessary
     * @param formatter the formatter to use, or {@code null} for the {@link #INSTANCE default}
     *
     * @return the formatted header elements
     */
    public static String formatElements(final HeaderElement[] elems, final boolean quote,
      final HeaderValueFormatter formatter) {
        return (formatter != null ? formatter : BasicHeaderValueFormatter.INSTANCE).formatElements(null,
          elems, quote).toString();
    }
    ...
    public static String formatHeaderElement(final HeaderElement elem, final boolean quote,
      final HeaderValueFormatter formatter) {
        return (formatter != null ? formatter : BasicHeaderValueFormatter.INSTANCE).formatHeaderElement(null,
          elem, quote).toString();
    }
     ...
     */
    public static String formatParameters(final NameValuePair[] nvps, final boolean quote,
      final HeaderValueFormatter formatter) {
        return (formatter != null ? formatter : BasicHeaderValueFormatter.INSTANCE).formatParameters(null,
          nvps, quote).toString();
    }
     ...
     * @param quote {@code true} to always format with a quoted value, {@code false} to use quotes only when
     *   necessary
     * @param formatter the formatter to use, or {@code null} for the {@link #INSTANCE default}
     *
     * @return the formatted name-value pair
     */
    public static String formatNameValuePair(final NameValuePair nvp, final boolean quote,
      final HeaderValueFormatter formatter) {
        return (formatter != null ? formatter : BasicHeaderValueFormatter.INSTANCE).formatNameValuePair(null,
          nvp, quote).toString();
    }
    ...
    /**
     * Actually formats the value of a name-value pair. This does not include a leading = character. Called
     * from {@link #formatNameValuePair formatNameValuePair}.
     *
     * @param buffer the buffer to append to, never {@code null}
     * @param value the value to append, never {@code null}
     * @param quote {@code true} to always format with quotes, {@code false} to use quotes only when
     *   necessary
     */
     ...
     *
     * @return {@code true} if the character is a separator, {@code false} otherwise
     */
