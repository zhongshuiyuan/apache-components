...
/**
 * Basic parser for lines in the head section of an HTTP message. There are individual methods for parsing a
 * request line, a status line, or a header line. The lines to parse are passed in memory, the parser does not
 * depend on any specific IO mechanism. Instances of this class are stateless and thread-safe. Derived classes
 * MUST maintain these properties.
 *
 * <p> Note: This class was created by refactoring parsing code located in various other classes. The author
 * tags from those other classes have been replicated here, although the association with the parsing code
 * taken from there has not been traced. </p>
 *
 * @since 4.0
 */
...
public class BasicLineParser
  implements LineParser {
    ...
    /**
     * A default instance of this class, for use as default or fallback. Note that {@link BasicLineParser} is
     * not a singleton, there can be many instances of the class itself and of derived classes. The instance
     * here provides non-customized, default behavior.
     *
     ...
     *
     * @param proto a version of the protocol to parse, or {@code null} for HTTP. The actual version is not
      ...
      throws ParseException {
        ...
        if (period == -1) {
            throw new ParseException(
              "Invalid protocol version number: " + buffer.substring(indexFrom, indexTo));
        }
        final int major;
        try {
            ...
            throw new ParseException(
              "Invalid protocol major version number: " + buffer.substring(indexFrom, indexTo));
        }
        i = period + 1;
        ...
        try {
            ...
            throw new ParseException(
              "Invalid protocol minor version number: " + buffer.substring(indexFrom, indexTo));
        }
     ...
     */
    protected RequestLine createRequestLine(final String method, final String uri,
        ...
        try {
            ...
            for (int j = 0; j < s.length(); j++) {
                if (!Character.isDigit(s.charAt(j))) {
                    throw new ParseException(
                      "Status line contains invalid status code: " + buffer.substring(indexFrom, indexTo));
                }
            }
            try {
