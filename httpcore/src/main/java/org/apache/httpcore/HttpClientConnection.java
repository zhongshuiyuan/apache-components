...
public interface HttpClientConnection
  extends HttpConnection {
    ...
    /**
     * Checks if response data is available from the connection. May wait for the specified time until some
     * data becomes available. Note that some implementations may completely ignore the timeout parameter.
     *
     * @param timeout the maximum time in milliseconds to wait for data
     *
     * @return true if data is available; false if there was no data available even after waiting for {@code
    ...
    /**
     * Receives the request line and headers of the next response available from this connection. The caller
     * should examine the HttpResponse object to find out if it should try to receive a response entity as
     * well.
     *
    ...
    /**
     * Receives the next response entity available from this connection and attaches it to an existing
     * HttpResponse object.
     *
