...
/**
 * Provides access to version information for HTTP components. Static methods are used to extract version
 * information from property files that are automatically packaged with HTTP component release JARs. <p> All
 * available version information is provided in strings, where the string format is informal and subject to
 * change without notice. Version information is provided for debugging output and interpretation by humans,
 * not for automated processing in applications. </p>
 *
 * @since 4.0
 */
public class VersionInfo {
     ...
     */
    protected VersionInfo(final String pckg, final String module, final String release, final String time,
      final String clsldr) {
    ...
    /**
     * Obtains the name of the versioned module or informal unit. This data is read from the version
    ...
    /**
     * Obtains the release of the versioned module or informal unit. This data is read from the version
     * information for the package.
     *
    ...
    /**
     * Obtains the timestamp of the versioned module or informal unit. This data is read from the version
     * information for the package.
     *
    ...
    /**
     * Obtains the classloader used to read the version information. This is just the {@code toString} output
     * of the classloader, since the version information should not keep a reference to the classloader
    ...
    public String toString() {
        final StringBuilder sb = new StringBuilder(
          20 + infoPackage.length() + infoModule.length() + infoRelease.length() + infoTimestamp.length() +
     ...
     * @param clsldr the classloader to load from, or {@code null} for the thread context classloader
     *
     * @return the version information for all packages found, never {@code null}
     */
     ...
     *
     * @param pckg the package for which to load version information, for example "org.apache.http". The
     *   package name should NOT end with a dot.
     * @param clsldr the classloader to load from, or {@code null} for the thread context classloader
     *
     * @return the version information for the argument package, or {@code null} if not available
     */
    public static VersionInfo loadVersionInfo(final String pckg, final ClassLoader clsldr) {
        ...
        try {
            ...
            final InputStream is =
              cl.getResourceAsStream(pckg.replace('.', '/') + "/" + VERSION_PROPERTY_FILE);
            if (is != null) {
                try {
     ...
     * @param info the map from string keys to string values, for example {@link Properties}
    ...
    /**
     * Sets the user agent to {@code "<name>/<release> (Java/<java.version>)"}. <p> For example:
     * <pre>"Apache-HttpClient/4.3 (Java/1.6.0_35)"</pre>
     *
     * @param name the component name, like "Apache-HttpClient".
     * @param pkg the package for which to load version information, for example "org.apache.http". The
     *   package name should NOT end with a dot.
     * @param cls the class' class loader to load from, or {@code null} for the thread context class loader
     *
     * @since 4.3
     */
